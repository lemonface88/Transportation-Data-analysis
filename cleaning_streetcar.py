...
streetcar_data = pd.read_csv('streetcar_data.csv')
streetcar_data["Date"] = pd.to_datetime(streetcar_data["Date"], format="%d-%b-%y").dt.strftime("%Y-%m-%d")

streetcar_data = streetcar_data.dropna()
...
streetcar_data["Time"] = streetcar_data["Time"].astype(str).str.strip()

# Ensure 'Time' column is properly formatted by extracting HH:MM only
streetcar_data["Time"] = streetcar_data["Time"].str.extract(r'(\d{1,2}:\d{2})')[0]

# Convert 'Time' column to datetime format and extract the time part
streetcar_data["Time"] = pd.to_datetime(streetcar_data["Time"], format="%H:%M", errors='coerce').dt.time


# step 2, remove all 0,0 min gap and min delays in the datasets

streetcar_data = streetcar_data[streetcar_data["Min Delay"] != 0]
streetcar_data = streetcar_data[streetcar_data["Min Gap"] != 0]
...
streetcar_data["Rush Hour Index"] = streetcar_data["Time"].apply(rush_hour_index)

print(streetcar_data.head(5))
...
merged_data = pd.merge(streetcar_data, weather_data, on='Date', how='inner')

print(merged_data.head(5))

merged_data.to_csv('streetcar_model_data.csv')
