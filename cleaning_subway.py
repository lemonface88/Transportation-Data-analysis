...
subway = pd.read_csv('subway_coor.csv')
...
subway = subway[subway["Min Gap"] != 0]

subway = subway.dropna(subset=['Time'])
subway['Time'] = pd.to_datetime(subway['Time'], errors='coerce').dt.time
...
subway["Rush Hour Index"] = subway["Time"].apply(rush_hour_index)
...
merged_data = pd.merge(subway, weather_data, on='Date', how='inner')

print(merged_data.head(5))

merged_data.to_csv('subway_model_data.csv')
